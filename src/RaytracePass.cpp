	...
	, mItrs(1)
{
	...
	mPrecomputeProgram.BindImage("rawVolume", dicom->GetTexture().Get(), 0, GL_TRUE);
	mPrecomputeProgram.BindImage("bakedVolume", mBakedVolumeTexture.Get(), 0, GL_TRUE);
	mPrecomputeProgram.UpdateUniform("scanResolution", dicom->GetScanSize());
	mPrecomputeProgram.Execute((bakeSize.x + 7) / 8, (bakeSize.y + 7) / 8, (bakeSize.z + 7) / 8);
