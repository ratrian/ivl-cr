...
void ComputeProgram::BindImage(std::string name, GLuint tex, GLuint level, GLboolean layered)
{
	const ImgBinding& bindInfo = mImgBindings.at(name);
	glBindImageTexture(bindInfo.binding, tex, level, layered, 0, bindInfo.access, bindInfo.format);
}
